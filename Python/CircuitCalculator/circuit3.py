...
def one(Frame):
    ...
    Frame.title("Example Curcuit 3")
    Frame.geometry("1500x750")

    bgImage = PhotoImage(file=r'CircuitThree.PNG').subsample(2)
    Label(Frame, image=bgImage).place(relwidth=1.4, relheight=1)
    ...
    def reset():
        ...
        VoltSour1.delete(0, END)
        chosen5_1.set('select')
        VoltSour2.delete(0, END)
    ...
    def solve():
        def complexnumber():
            ...
            call_resultCSource(label_Result6, Csrc, CurrSour, chosen6_1)
            call_resultVSource(label_Result5, Vsrc1, VoltSour1, chosen5_1)
            call_resultVSource(label_Result7, Vsrc2, VoltSour2, chosen7_1)
        ...
        def output():
            ...
            d = complex(d1_1, d1_2)
            C = float(CurrSour.get()) * translate(chosen6_1.get())
            V1 = float(VoltSour1.get()) * translate(chosen5_1.get())
            V2 = float(VoltSour2.get()) * translate(chosen7_1.get())
            num1 = np.array([[0, -b, 0, 0],
                             [V1, 0, c, -c],
                             [V2, b, -c, c + d],
                             [C, -1, 0, 1]])
            num2 = np.array([[a + b, 0, 0, 0],
                             [0, V1, c, -c],
                             [-b, V2, -c, c + d],
                             [0, C, 0, 1]])
            num3 = np.array([[a + b, -b, 0, 0],
                             [0, 0, V1, -c],
                             [-b, b, V2, c + d],
                             [0, -1, C, 1]])
            num4 = np.array([[a + b, -b, 0, 0],
                             [0, 0, c, V1],
                             [-b, b, -c, V2],
                             [0, -1, 0, C]])
            den = np.linalg.det(np.array([[a + b, -b, 0, 0],
                                          [0, 0, c, -c],
                                          [-b, b, -c, c + d],
                                          [0, -1, 0, 1]]))
        ...
        Pass = (test(Re1, Im1) == 'pass' or test(Re2, Im2) == 'pass' or test(Re3, Im3) == 'pass'
                or test(Re4, Im4) == 'pass' or test1(Vsrc1) == 'pass' or test1(Vsrc2) == 'pass' or test1(Csrc) == 'pass')
        Fail = (test(Re1, Im1) == 'fail' or test(Re2, Im2) == 'fail' or test(Re3, Im3) == 'fail'
                or test(Re4, Im4) == 'fail' or test1(Vsrc1) == 'fail' or test1(Vsrc2) == 'fail' or test1(Csrc) == 'fail')
        if Fail:
        ...
        if test(Re1, Im1) == 'fail':
            ...
            CurrSour.delete(0, END)
            chosen6_1.set('select')
        elif test1(Vsrc1) == 'fail':
            VoltSour1.delete(0, END)
            chosen5_1.set('select')
        elif test1(Vsrc2) == 'fail':
            VoltSour2.delete(0, END)
            chosen7_1.set('select')
        elif Pass:
            output()
    ...
    Label(Frame, text="R∠ϴ(deg)", font=('Helvetica',20)).grid(row=10, column=1)
    ...
    label_Result1 = Entry(Frame, width=21, borderwidth=3, justify='center')
    ...
    label_Result3.place(x=1080, y=470)
    ...
    label_Result4.place(x=1170, y=685)

    # ALL about Vsource1
    [chosen5_1, multiplier5_1] = Multiply()
    [Vsrc1, VoltSour1] = Entry_Source()
    label_Result5 = Entry(Frame, width=10, borderwidth=3, justify='center')
    [call_result5, buttonCalc5] = EnterButtonSource(call_resultVSource, label_Result5, Vsrc1, VoltSour1, chosen5_1)

    # POSITIONS for Vsource1
    VoltSour1.grid(row=6, column=1)
    multiplier5_1.grid(row=6, column=2, sticky=N+E+W+S)
    Label(Frame, text="Voltage Source 1 = ").grid(row=6, column=0, sticky=E)
    label_Result5.place(x=715, y=625)

    # ALL about Vsource2
    [chosen7_1, multiplier7_1] = Multiply()
    [Vsrc2, VoltSour2] = Entry_Source()
    label_Result7 = Entry(Frame, width=10, borderwidth=3, justify='center')
    [call_result7, buttonCalc7] = EnterButtonSource(call_resultVSource, label_Result7, Vsrc2, VoltSour2, chosen7_1)

    # POSITIONS for Vsource2
    VoltSour2.grid(row=7, column=1)
    multiplier7_1.grid(row=7, column=2, sticky=N+E+W+S)
    Label(Frame, text="Voltage Source 2 = ").grid(row=7, column=0, sticky=E)
    label_Result7.place(x=1240, y=82)
    ...
    [call_result6, buttonCalc6] = EnterButtonSource(call_resultCSource, label_Result6, Csrc, CurrSour, chosen6_1)

    # POSITIONS for Csource
    CurrSour.grid(row=8, column=1)
    multiplier6_1.grid(row=8, column=2, sticky=N+E+W+S)
    Label(Frame, text="Current Source = ").grid(row=8, column=0, sticky=E)
    label_Result6.place(x=1229, y=350)

    # OUTPUT
    Label(Frame, text=" ").grid(row=6, column=3, sticky=E)
    # EVALUATE BUTTON
    ...
    # MESH I1
    Mesh_1 = Entry(Frame, width=20, borderwidth=3, justify='center')
    Mesh_1.place(x=835, y=235)
    Label(Frame, text="Mesh Current I1: ").grid(row=11, column=0, sticky=E)
    Angle1 = Entry(Frame, width=25, borderwidth=3, justify='center')
    Angle1.grid(row=11, column=1, columnspan=1, sticky=W)
    # MESH I2
    Mesh_2 = Entry(Frame, width=20, borderwidth=3, justify='center')
    Mesh_2.place(x=1208, y=225)
    Label(Frame, text="Mesh Current I2: ").grid(row=12, column=0, sticky=E)
    Angle2 = Entry(Frame, width=25, borderwidth=3, justify='center')
    Angle2.grid(row=12, column=1, columnspan=1, sticky=W)
    # MESH I3
    Mesh_3 = Entry(Frame, width=20, borderwidth=3, justify='center')
    Mesh_3.place(x=839, y=560)
    Label(Frame, text="Mesh Current I3: ").grid(row=13, column=0, sticky=E)
    Angle3 = Entry(Frame, width=25, borderwidth=3, justify='center')
    Angle3.grid(row=13, column=1, columnspan=1, sticky=W)
    # MESH I4
    Mesh_4 = Entry(Frame, width=20, borderwidth=3, justify='center')
    Mesh_4.place(x=1210, y=565)
    Label(Frame, text="Mesh Current I4: ").grid(row=14, column=0, sticky=E)
    Angle4 = Entry(Frame, width=25, borderwidth=3, justify='center')
    Angle4.grid(row=14, column=1, columnspan=1, sticky=W)
