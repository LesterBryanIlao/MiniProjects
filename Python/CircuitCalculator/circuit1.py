...
def one(Frame):
    ...
    Frame.title("Example Curcuit1")
    Frame.geometry("1360x750")

    bgImage = PhotoImage(file=r'CircuitOne.PNG').subsample(2)
    Label(Frame, image=bgImage).place(relwidth=1, relheight=1.3)
    ...
    def solve():
        def complexnumber():
            ...
            call_resultVSource(label_Result5, Vsrc, VoltSour, chosen5_1)
        ...
        def output():
            ...
            d = complex(d1_1, d1_2)
            V = float(VoltSour.get()) * translate(chosen5_1.get())
            C = float(CurrSour.get()) * translate(chosen6_1.get())
            num1 = np.array([[-V, b], [C, 1]])
            num2 = np.array([[a, -V], [-1, C]])
            den = np.linalg.det(np.array([[a, b], [-1, 1]]))
        ...
        Pass = (test(Re1, Im1) == 'pass' or test(Re2, Im2) == 'pass' or test(Re3, Im3) == 'pass'
                or test(Re4, Im4) == 'pass' or test1(Vsrc) == 'pass' or test1(Csrc) == 'pass')
    ...
    label_Result1.place(x=221, y=290)
    ...
    label_Result2.place(x=595, y=290)
    ...
    label_Result3.place(x=975, y=290)
    ...
    label_Result4.place(x=975, y=650)
    ...
    [call_result5, buttonCalc5] = EnterButtonSource(call_resultVSource, label_Result5, Vsrc, VoltSour, chosen5_1)

    # POSITIONS for Vsource
    VoltSour.grid(row=6, column=1)
    multiplier5_1.grid(row=6, column=2, sticky=N+E+W+S)
    Label(Frame, text="Voltage Source = ").grid(row=6, column=0, sticky=E)
    label_Result5.place(x=885, y=530)
    ...
    CurrSour.grid(row=7, column=1)
    multiplier6_1.grid(row=7, column=2, sticky=N+E+W+S)
    Label(Frame, text="Current Source = ").grid(row=7, column=0, sticky=E)
    label_Result6.place(x=500, y=530)

    # OUTPUT
    # EVALUATE BUTTON
    EvalButton = Button(Frame, text='Evaluate', width=6, borderwidth=4, command=solve)
    EvalButton.config(bg='green3')
    EvalButton.grid(row=6, column=3, columnspan=1, sticky=N+E+W+S)

    # RESET BUTTON
    ResetButton = Button(Frame, text='Reset', width=6, borderwidth=4, command=reset)
    ResetButton.config(bg='red')
    ResetButton.grid(row=7, column=3, columnspan=1, sticky=N + E + W + S)

    # MESH I1
    Mesh_1 = Entry(Frame, width=30, borderwidth=3, justify='center')
    Mesh_1.place(x=221, y=500)
    ...
    # MESH I2
    Mesh_2 = Entry(Frame, width=30, borderwidth=3, justify='center')
    Mesh_2.place(x=595, y=500)
    ...
    # MESH I3
    Mesh_3 = Entry(Frame, width=30, borderwidth=3, justify='center')
    Mesh_3.place(x=995, y=500)
